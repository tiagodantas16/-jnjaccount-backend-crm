    ...
    <div class="content">
      ...
      <div class="content-data">
        ...
        <div class="stat">
          ...
          <span id="two">EM ANÁLISE</span>      
          <span id="three">CADASTRO FINALIZADO</span>
          <span id="for"><a href="https://google.com.br" target="_blank">AGENDE TREINAMENTO</a></span>
        </div>
        <div class="line">
          <div class="create"></div>
          <div class="hover-create"></div>
...
module.exports = {
  async ToSendClientConfirm(request, response) {
    const { email, send } = request.body;
    
    if (send === 'request') {
      var Send = layout.request
    }
    if(send === 'status01') {
      var Send = layout.status01
    }
    if(send === 'status02') {
      var Send = layout.status02
    }
    if(send === 'status03') {
      var Send = layout.status03
    }
